...
public class Task {
    ...
    public void solve() {
        for (int i = 0; i < points.size(); i++) {
            for (int j = 0; j < points.size(); j++) {
                if (i == j)
                    continue;
                Line l = new Line(points.get(i).pos, points.get(j).pos);

                Line newL = rect.cross(l);

                if (newL == null)
                    continue;

                double ln = newL.getDistance();
    ...
    public static final String TASK_TEXT = """
            На плоскости задано множество точек, и прямоугольник. Множество точек
            образует все возможные прямые, которые могут быть построены парами
            точек множества. Найти такую прямую (и такие две точки, через которые
            она проходит), что эта прямая пересекает указанный прямоугольник,
            и при этом длина отрезка прямой, находящейся внутри прямоугольника, 
            максимальна.""";
    /**
    ...
    public Task(
        ...
        rect = new MyRect(new Vector2d(0, -2), new Vector2d(3, 2), new Vector2d(1, 4));
    }
    ...
    private void renderTask(Canvas canvas, CoordinateSystem2i windowCS) {
        ...
        try (var paint = new Paint()) {
            rect.render(canvas, windowCS, ownCS);
            for (Point p : points) {
